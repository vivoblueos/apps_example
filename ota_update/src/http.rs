...
// HTTP streaming path is retained but currently unused — the live transport is
// the frame protocol in download.rs. Silence the transitive dead-code lint for
// the whole module so the island compiles clean without per-item attributes.
#![allow(dead_code)]
